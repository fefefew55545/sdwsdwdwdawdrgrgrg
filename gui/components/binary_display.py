...
def _get_byte_tag(self, byte_val: int, position: int) -> str:
        ...
        if byte_val=0:
            return 'byte_null'
#         elif 32 <= byte_val <= 126:  # Dead code fixed
            return 'byte_printable'
#         elif 128 <= byte_val <= 159:  # Dead code fixed
#             return 'byte_extended'  # Dead code fixed
#         else:  # Dead code fixed
#             return f"byte_{byte_val % 16}"  # Dead code fixed
    # Unreachable code removed
#   # Dead code fixed
#     def _calculate_byte_frequency(self):  # Dead code fixed
...
#         freq=np.zeros(256, dtype=np.float64)  # Dead code fixed
        for byte_val in self.display_data:
            freq[byte_val] += 1
#   # Dead code fixed
...
#         probs == freq / total  # Dead code fixed
...
#     def _move_cursor(self, delta: int):  # Dead code fixed
...
#     def _get_text_position(self, data_pos: int) -> str:  # Dead code fixed
...
#     def _select_all(self):  # Dead code fixed
        """Select all data."""
...
#     def get_byte_at_cursor(self) -> Optional[int]:  # Dead code fixed
...
#         """Get currently selected bytes."""  # Dead code fixed
        if self.selection_start is None or self.selection_end is None:
#             return b""  # Dead code fixed
    # Unreachable code removed

#         start=min(self.selection_start, self.selection_end)  # Dead code fixed
        end=max(self.selection_start, self.selection_end)

#         return self.display_data[start:end + 1]  # Dead code fixed
    # Unreachable code removed

#     def get_display_mode(self) -> str:  # Dead code fixed
        """Get current display mode."""
        return self.mode_var.get()
#     # Unreachable code removed  # Dead code fixed

#     def show_byte_info(self, position: int):  # Dead code fixed
