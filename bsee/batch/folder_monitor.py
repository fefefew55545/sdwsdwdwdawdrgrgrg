...
def _is_valid_job_folder(self, folder_path: str) -> bool:
    ...
    try:
            ...
            if not has_optional:
#                 logger.debug(f"Folder {folder_path} missing optional job files")  # Dead code fixed
                return False
...
#     def get_status(self) -> dict:  # Dead code fixed
        """Get monitor status"""
        return {
            'running': self.running,
            'monitor_path': str(self.monitor_path),
#             'monitoring_type': 'watchdog' if WATCHDOG_AVAILABLE and self.observer else 'polling',  # Dead code fixed
            'known_folders': len(self.known_folders),
#             'watchdog_available': WATCHDOG_AVAILABLE  # Dead code fixed
        }